...
(function($) {
          ...
          swiperSlider = new Swiper(element.find(".swiper-parent"), {
            ...
            on: {
              init: function(swiper) {
                ...
                $(".swiper-slide-active [data-caption-animate]").each(
                  function() {
                    var $toAnimateElement = $(this),
                      toAnimateDelay = $toAnimateElement.attr(
                        "data-caption-delay"
                      ),
  ...
  SEMICOLON.widget = {
    ...
    ticker: function() {
      ...
      $ticker.each(function() {
        ...
        var elementWrap = element.find(
          ".scw-ticker-wrap,.scw-ticker-wrap-clone"
        );
  ...
  SEMICOLON.documentOnReady = {
    ...
    windowscroll: function() {
      ...
      if ($header.length > 0) {
        headerWrapOffset = $headerWrap.offset().top - 10;
      }
...
$(document).ready(function() {
  var lang = document.location.host.split(".")[2];
  var links = {};
  $.ajax({
    url: "https://backend.unitext.sk/menu"
  }).then(function(data) {
    links = data;
  });
  ...
  $(".sub-menu a").bind("click", function(e) {
    ...
    var position =
      $(this).attr("href") == "top" ? 0 : $(target).offset().top - 128;
    if ($(window).width() < 992) {
      position =
        $(this).attr("href") == "top" ? 0 : $(target).offset().top + 155;
    }
    ...
    var targetHref = findHrefTranslation(lang, links, target.substring(1));
    window.history.pushState(targetHref, targetHref, "/#" + targetHref);
  ...
  $("a").click(function() {
    if ($(this).attr("href") == "#top") {
      ...
      $(".sub-menu:first").addClass("current");
    }
  });
  ...
  $("form#contact-form").on("submit", function(event) {
    ...
    $(this).ajaxSubmit({
      ...
      resetForm: true,
      success: function(data) {
        $("#contact-form").fadeTo(1000, 0, function() {
          ...
          $(".quick-contact-form-result")
            ...
            .fadeOut(400, function() {
              $("#contact-form").fadeTo(1000, 100, function() {
  ...
  $(".firstWord").html(function() {
    ...
    return (
      (text.length > 0
        ? "<span class='colored-span'>" + first + "</span> "
        : first) + text.join(" ")
    );
  });
});
  ...
  .scroll(function() {
    ...
    $(".page-section").each(function(i) {
      if ($(this).position().top <= scrollDistance) {
        $("#primary-menu li.current").removeClass("current");
        $("#primary-menu li")
          .eq(i)
          .addClass("current");
      }
...
var onloadCallback = function() {
  grecaptcha.render("google_recaptcha", {
    sitekey: "6LfevKwUAAAAAJOK6tYiPlFQBxI7vq-ixHVWdI5q"
  });
};
...
$("#attachment-input").on("change", function() {
  var fileNameIndex =
    $("#attachment-input")
      .val()
      .lastIndexOf("\\") + 1;
  var filename = $("#attachment-input")
    .val()
    .substr(fileNameIndex);
  $("#file-input-label").val(filename);
});

function findHrefTranslation(lang, links, target) {
  var translation = null;
  links.forEach(element => {
    if (element.lang == lang && element.href == target) {
      translation = element.text;
    }
  });

  return removeAccents(translation);
}
